...
impl JwtMiddleware {
    ...
    pub fn load_public_key(&self) -> UpliftResult<DecodingKey> {
        let public_key_pem = fs::read_to_string(&self.public_key_path)
            .map_err(|e| UpliftError::AuthFail)?;
    ...
    pub fn validate_jwt(&self, token: &str) -> UpliftResult<Claims> {
...
pub async fn validate_jwt_middleware<B>(req: axum::http::Request<B>, jwt_middleware: JwtMiddleware, next: Next<B>) -> UpliftResult<Response> {
